...
<?php
	...
	public function access_meta_box()
	{
		...
		$ctx = $this->ctx;
		$opts = \get_option('umich_oidc_settings');
		$post_types = [];
		foreach ($opts['restrict_types'] as $type) {
			$post_types[] = \trim($type, '_');
		}
		\add_meta_box('umich_oidc_access_meta', 'UMich OIDC access', array($this, 'access_meta_callback'), $post_types, 'side', 'high');
	}
